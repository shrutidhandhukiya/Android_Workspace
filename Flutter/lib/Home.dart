                        ...
                        Container(
                            ...
                            child:GridView.builder(
                             ...
                             itemCount: provider.products.length==0?0:provider.products.length,itemBuilder:(BuildContext context, int index){
                               DrawerWidget();
                               return ProductCard(product: products[index], context: context, set: this);
...
LayoutWidgetOfsidebar(int index, CategoriesProvider provider) {
    return  Column(
      children: [
        Container(
          child: ClipRRect(
            ...
            children: [
                GestureDetector(
                    onTap: ()async{
                      ...
                      // cart.add(AddtoCardItens(product,product.sected??0));

                      var cartresponse=await httpService().sendApiRequest(url: "{{URL}}api/rest/cart", headers: {
                        "Authorization":"Bearer b1e935e5bd3fdc34b5de35c3f8917373c9a68151"},
                          body: {}, method: "post");

                      // cart.add(GetCartModel(success: , error: error, data: data));
